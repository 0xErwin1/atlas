...
- **Permissions** are resource-sharing style (not IAM): grants `(principal, resource, role)` with `viewer`/`editor`/`admin` roles inheriting down `workspace > project > folder > document|board`, visibility as sugar, default deny, agents capped at `editor` and never managing grants. Every protected route declares its target resource + minimum role via the `Authorized<…>` extractor; an undeclared protected route is caught by the route-protection audit test.
