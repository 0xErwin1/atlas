...
- **Shared components — reuse, never duplicate (non-negotiable).** Use the design-system primitives instead of re-implementing dropdowns, menus, toggles, confirmations, or rows per panel: `Dropdown` (single-select), `Popover` (anchored surface), `ConfirmDialog`, `FormField` + `validateForm`, `ExpandableRow` (collapsed-summary + inline manage panel). The moment a visual/behavioral pattern recurs, extract one component and have every call site use it; duplicated markup/CSS across components is a defect to remove, not extend. Full rule in `CODE_STYLE.md` → TypeScript / Vue → Patterns.
