...
- **Reuse, don't reimplement — no duplication (non-negotiable).** Before writing UI, use the shared primitive; never copy its markup, styles, or behavior into a panel. Canonical UI primitives:
  - single-select dropdown → `Dropdown` (`src/components/ui/Dropdown.vue`); anchored menu / popover surface → `Popover`; confirmation → `ConfirmDialog`; form field + validation → `FormField` + `validateForm`;
  - settings table shell → `SettingsTable`; expandable settings row (collapsed summary + inline manage panel, whole-row click) → `ExpandableRow`; panel header (title + optional subtitle + actions slot) → `PanelHeader`; compact row-action button (`tone`/`iconOnly`/`disabled`) → `RowAction` — all under `src/components/settings/`;
  - empty state (full + `compact` variant) → `EmptyState` (`src/components/states/EmptyState.vue`).
