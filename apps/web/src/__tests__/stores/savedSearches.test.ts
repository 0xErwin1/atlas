...
describe('useSavedSearchesStore (SE20)', () => {
  ...
  it('load GETs the collection and fills items', async () => {
    ...
    expect(GET).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/saved-searches', {
      params: { path: { ws: 'ws-1' } },
    });
  ...
  it('create POSTs {name,query} and returns sorted DTO on success', async () => {
    ...
    expect(POST).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/saved-searches', {
      params: { path: { ws: 'ws-1' } },
  ...
  it('rename PATCHes {name} and replaces the item in items (SE18)', async () => {
    ...
    expect(ok).toBe(true);
    expect(PATCH).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/saved-searches/{id}', {
  ...
  it('remove DELETEs and filters the item from items (SE19)', async () => {
    ...
    expect(ok).toBe(true);
    expect(DELETE).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/saved-searches/{id}', {
      params: { path: { ws: 'ws-1', id: 's1' } },
    });
    expect(store.items).toHaveLength(1);
