...
describe('useWorkspaceStore', () => {
  ...
  it('renameProject PATCHes the project and refreshes the list', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockPatch).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/projects/{project_slug}', {
      params: { path: { ws: 'atlas', project_slug: 'roadmap' } },
      body: { name: 'Roadmap 2' },
    });
    expect(mockGet).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/projects', {
  ...
  it('deleteProject DELETEs the project and refreshes the list', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockDelete).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/projects/{project_slug}', {
      params: { path: { ws: 'atlas', project_slug: 'roadmap' } },
    });
    expect(mockGet).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/projects', {
      params: { path: { ws: 'atlas' }, query: { limit: 200 } },
  ...
  it('createWorkspace POSTs, refreshes the list and switches to the new one', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith('/api/v2/acta/workspaces', { body: { name: 'New WS' } });
  ...
  it('renameWorkspace PATCHes the workspace and reflects the new name in the cached list', async () => {
    ...
    expect(ok).toBe(true);
    expect(mockPatch).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}', {
  ...
  it('loadAdminWorkspaces GETs the admin list and stores it', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith('/api/v2/acta/admin/workspaces');
