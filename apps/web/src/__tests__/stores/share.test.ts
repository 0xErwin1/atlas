...
describe('useShareStore — workspace resource (REQ-W26/W27)', () => {
  ...
  it('load populates grants from the workspace grants list', async () => {
    ...
    expect(GET).toHaveBeenCalledWith('/api/v2/custos/workspaces/{ws}/grants', {
  ...
  it('removeGrant DELETEs and re-fetches', async () => {
    ...
    expect(ok).toBe(true);
    expect(DELETE).toHaveBeenCalledWith('/api/v2/custos/workspaces/{ws}/grants/{grant_id}', {
  ...
  it('loadMembers populates members from the workspace members endpoint', async () => {
    ...
    expect(GET).toHaveBeenCalledWith('/api/v2/acta/workspaces/{ws}/members', {
      params: { path: { ws: 'acme' } },
    });
...
describe('useShareStore — project resource dispatch', () => {
  ...
  it('load calls the project grants endpoint when resource is a project', async () => {
    ...
    expect(GET).toHaveBeenCalledWith('/api/v2/custos/workspaces/{ws}/projects/{project_slug}/grants', {
  ...
  it('addGrant POSTs to project grants endpoint for a project resource', async () => {
    ...
    expect(ok).toBe(true);
    expect(POST).toHaveBeenCalledWith('/api/v2/custos/workspaces/{ws}/projects/{project_slug}/grants', {
