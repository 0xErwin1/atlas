...
export const useTagsStore = defineStore('tags', () => {
  ...
  async function load(ws: string, force = false): Promise<void> {
    ...
    const { data, error: apiError } = await acta.GET('/api/v2/acta/workspaces/{ws}/tags', {
      params: { path: { ws } },
    });
  ...
  async function loadUsed(ws: string, force = false): Promise<void> {
    ...
    const { data, error: apiError } = await acta.GET('/api/v2/acta/workspaces/{ws}/tags/used', {
      params: { path: { ws } },
    });
  ...
  async function ensure(ws: string, name: string): Promise<TagDto | null> {
    ...
    const { data, error: apiError } = await acta.POST('/api/v2/acta/workspaces/{ws}/tags', {
      params: { path: { ws } },
      body: { name: trimmed },
    });
  ...
  async function update(
    ...
    const generation = bindWorkspace(ws);
    const { data, error: apiError } = await acta.PATCH('/api/v2/acta/workspaces/{ws}/tags/{tag_id}', {
  ...
  async function remove(ws: string, id: string): Promise<boolean> {
    const generation = bindWorkspace(ws);
    const { error: apiError } = await acta.DELETE('/api/v2/acta/workspaces/{ws}/tags/{tag_id}', {
      params: { path: { ws, tag_id: id } },
    });
