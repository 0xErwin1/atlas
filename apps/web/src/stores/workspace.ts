...
import type { CacheInvalidationScope } from '@/api/wrapper';
...
export const useWorkspaceStore = defineStore('workspace', () => {
  ...
  async function loadWorkspaces(): Promise<string | null> {
    ...
    const { data, error } = await acta.GET('/api/v2/acta/workspaces');
  ...
  async function createWorkspace(name: string): Promise<string | null> {
    const requestGeneration = ++workspaceLoadGeneration;
    const requestSessionGeneration = auth.sessionGeneration;
    const { data, error: apiError } = await acta.POST('/api/v2/acta/workspaces', {
      body: { name },
    });
    ...
    const list = await acta.GET('/api/v2/acta/workspaces');
    if (!isCurrentWorkspaceRequest(requestGeneration, requestSessionGeneration)) return null;
  ...
  async function loadProjects(ws: string, options: { preserveOnError?: boolean } = {}): Promise<void> {
    ...
    const { items, error: loadError } = await collectPaged<ProjectDto>((cursor) =>
      acta.GET('/api/v2/acta/workspaces/{ws}/projects', {
  ...
  async function createProject(ws: string, name: string): Promise<string | null> {
    ...
    const { data, error: apiError } = await acta.POST('/api/v2/acta/workspaces/{ws}/projects', {
      params: { path: { ws } },
  ...
  async function renameProject(ws: string, slug: string, name: string): Promise<boolean> {
    const { error: apiError } = await acta.PATCH('/api/v2/acta/workspaces/{ws}/projects/{project_slug}', {
  ...
  async function deleteProject(ws: string, slug: string): Promise<boolean> {
    const { error: apiError } = await acta.DELETE('/api/v2/acta/workspaces/{ws}/projects/{project_slug}', {
      params: { path: { ws, project_slug: slug } },
    });
  ...
  async function renameWorkspace(ws: string, name: string): Promise<boolean> {
    const requestGeneration = ++workspaceLoadGeneration;
    const requestSessionGeneration = auth.sessionGeneration;
    const { data, error: apiError } = await acta.PATCH('/api/v2/acta/workspaces/{ws}', {
      params: { path: { ws } },
      body: { name },
    });
  ...
  async function updateWorkspaceSlug(ws: string, slug: string): Promise<boolean> {
    const requestGeneration = ++workspaceLoadGeneration;
    const requestSessionGeneration = auth.sessionGeneration;
    const { data, error: apiError } = await acta.PATCH('/api/v2/acta/admin/workspaces/{ws}', {
      params: { path: { ws } },
      body: { slug },
    });
  ...
  async function deleteWorkspace(ws: string): Promise<boolean> {
    const { error: apiError } = await acta.DELETE('/api/v2/acta/admin/workspaces/{ws}', {
      params: { path: { ws } },
    });
  ...
  async function loadAdminWorkspaces(): Promise<void> {
    const { data, error: apiError } = await acta.GET('/api/v2/acta/admin/workspaces');
  ...
  async function updateMemberRole(ws: string, userId: string, role: string): Promise<boolean> {
    const { error: apiError } = await acta.PATCH('/api/v2/acta/workspaces/{ws}/members/{user_id}', {
  ...
  async function removeMember(ws: string, userId: string): Promise<boolean> {
    const { error: apiError } = await acta.DELETE('/api/v2/acta/workspaces/{ws}/members/{user_id}', {
      params: { path: { ws, user_id: userId } },
    });
  ...
  async function loadAssignableUsers(ws: string): Promise<void> {
    ...
    const { data, error: apiError } = await acta.GET('/api/v2/acta/workspaces/{ws}/assignable-users', {
      params: { path: { ws } },
    });
  ...
  async function addMember(ws: string, userId: string, role: string): Promise<boolean> {
    const { error: apiError } = await acta.POST('/api/v2/acta/workspaces/{ws}/members', {
      params: { path: { ws } },
