...
describe('ScopeGrid', () => {
  it('forwards each scope token and gives each checkbox a concrete accessible name', () => {
    const wrapper = mount(ScopeGrid, { props: { modelValue: ['acta::tasks::read'] } });
    const checkbox = wrapper.get<HTMLInputElement>('input[data-scope="acta::tasks::read"]');
  ...
  it('emits the canonical selection when a capability is requested', async () => {
    ...
    await wrapper.get('input[data-scope="acta::tasks::create"]').setValue(true);

    expect(wrapper.emitted('update:modelValue')).toEqual([[['acta::tasks::create']]]);
  });
});
