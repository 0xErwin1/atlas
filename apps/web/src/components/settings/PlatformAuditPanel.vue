...
// accepts a single `action` verb, so a multi-verb category is applied
// client-side after fetching; "All actions" clears it.
...
function observeSentinel(): void {
  ...
  observer = new IntersectionObserver((records) => {
    for (const record of records) {
      if (record.isIntersecting) void store.loadMorePlatform();
    }
  });
