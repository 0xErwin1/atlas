...
type ContextState =
  ...
  | { kind: 'board'; boardId: string; currentName: string; archived: boolean };
...
const boardMenuItems = computed<MenuItem[]>(() => {
  ...
  const { boardId, currentName, archived } = state;
  return [
    { header: true, label: archived ? `${currentName} · archived` : currentName },
