...
python3 - "$manifest" "$gate_module" "$tauri_config" "$launch_controller" "$release_audit" "$flake" <<'PY'
import pathlib
import sys
...
flake = pathlib.Path(sys.argv[6]).read_text()
