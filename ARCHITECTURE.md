...
#### Access dual-write and the membership projection (V2-E7 S4)

- **Every V1 access write leaves V2 rows behind, in the same transaction** (`authz::v2_access`, ACTA-WS-3). An `owner` membership sets `acta.workspaces.owner_principal_id` and grants the custom role `acta:workspace-owner` on the workspace (created on first use per product: admin@1's Acta actions plus `workspace::transfer` and `workspace::delete`, never a Custos action); an `admin` membership grants the custom role `acta:workspace-admin` on the workspace (admin@1's Acta actions, created on first use the same way); a `member` membership grants nothing, since the `members` principal set is derived from `workspace_memberships`. Re-roling replaces the workspace role grant, removal revokes it; grants made on inner resources stay, as V1 permission grants do. `owner_principal_id` follows the principal that became owner and, when that principal loses the role, the oldest remaining owner membership. Admin memberships are a custom role in V2 so shares and memberships never collide: membership writes touch only the two membership roles, share writes only built-in roles. Both roles are found or created with `ON CONFLICT DO NOTHING`, so a concurrent first use never aborts the request's transaction. The dual-write finds them by name, so the Custos role routes reserve the `acta:workspace-` prefix: no custom role can be created with it, renamed into it, or renamed out of it (422).
- **V1 permission grants map one-to-one.** `(user | api_key | group, resource, role)` becomes a V2 grant of `role@1` on the same `ResourceRef` with `created_by = actor`; the subject is the user's principal, the key's principal (a personal key already carries its owner's principal), or the group. A repeated V1 upsert replaces the V2 row; a V1 delete revokes it. A project's creator grant is mirrored the same way.
- **Project visibility is a set grant.** `workspace(role)` grants `role@1` on the project to `acta::workspace::<id>::members`; `private` revokes it; `public` is treated as `private` (MIG-4). Only projects carry visibility in V1. The seeded projects carry it too: a new workspace's `General` project is created in the workspace's own transaction with its grant, and the dev seed's `Sandbox` project with its grant in one transaction. The root workspace of first boot and the desktop gate's ephemeral workspace seed no project; their owner memberships are written through the repositories, not the routes, and are not dual-written: the root user is authorized by V2 without grants, and the ephemeral workspace exists only behind the test seam. A new workspace's status templates and default board stay best-effort after the commit, as in V1.
- **Known limitation until the S5 cutover.** A V1 share write (create or delete) replaces or revokes every built-in grant with the same subject and target, including one a platform admin made directly through the V2 `/grants` route. It ends at S5, when the V1 share routes write V2 directly and the dual-write goes away.
- **Projection.** `acta.workspace_members(workspace_id, principal_id, role, source, updated_at)` is maintained by the same writes and read by nothing yet (S5 switches workspace reachability to it). No backfill of existing rows happens in S4.
- **Delegation check in audit mode.** After each grant-creating write (including the members visibility grant of a new project; the creator grant of a new project and the owner grant of a new workspace are self-bootstrap writes and skip it), GRANT-3 is evaluated, detached from the request after the commit and bounded to 100 ms (a timeout counts as reason `unavailable`), through `effective_actions` and `can_delegate` for non-admin actors; a would-be refusal is logged under `authz.v2_access` (`v2_access.would_refuse`) and counted in `atlas_v2_access_would_refuse_total{operation, reason}`, never returned, because V2 authority lags V1 until the backfill (D-E7S4-5).
