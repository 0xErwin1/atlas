...
/// instead. `atlas_server` constructs it as `WorkspaceScope(ctx.workspace_id.0)`
/// at the composition layer, so the same uuid reaches the same predicate as the
/// pre-move `WorkspaceCtx`-scoped query.
#[async_trait]
