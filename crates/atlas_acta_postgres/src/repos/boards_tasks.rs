//! Repository implementations for the `board`, `task`, `task_reference`,
//! `task_assignee`, `task_checklist_item`, and `task_activity` ports, moved
//! from `atlas_server::persistence::repos::boards_tasks` (S4 PR7).
//!
//! Raw joins into `custos.api_keys` (assignee lookups) are unchanged SQL
//! text, not a Rust-level `atlas_custos` dependency, so they move with the
//! file per design D6 — the same discipline PR6 documented for
//! `WorkspaceRepo::list_for_api_key`. `PgOutboxRepo` (`outbox.rs`) moved
//! alongside this file in the same PR: every mutation here emits a domain
//! event through it, and it has no Custos dependency of its own.
...
// `atlas_server`). Duplicated as a local, byte-identical constant.
mod acta_conflict {
...
use crate::entities::boards_tasks::{
...
use crate::entities::status_templates::status_template;
...
use crate::repos::outbox::PgOutboxRepo;
