//! Repository implementation for the `workspace_status_templates` port,
//! moved from `atlas_server::persistence::repos::status_templates`
//! (orchestrator-mandated addition to S4 PR9).
//!
//! No Custos dependency: `workspace_status_templates` carries no
//! `custos.*` foreign key, and neither this repo nor
//! `platform_status_templates.rs` composes `append_resource_deleted_in` or
//! `PgOutboxRepo`.
...
// `atlas_server`). Duplicated as a local, byte-identical constant, same
// pattern `boards_tasks.rs` already established in this crate.
mod acta_conflict {
...
use crate::entities::status_templates::{status_template, status_template_from};
