use crate::entities::comments::{comment, comment_link, comment_link_event, comment_link_from};
...
/// Internal test seam for proving comment-link mutations commit as one
/// transaction. Relocated here from `atlas_server::services::comment_service`
/// (S4 PR8): it is threaded into `PgCommentLinkRepo::
/// replace_for_comment_with_fault_in` as a parameter, so it must live in
/// whichever crate defines that method's type — `CommentService` (still in
/// `atlas_server`) now imports this definition rather than defining its own,
/// keeping the type identical across both crates for the equality checks in
/// this file (`fault == Some(CommentMutationFault::...)`), which duplicating
...
impl PgCommentLinkRepo {
    ...
    pub async fn replace_for_comment_with_fault_in(
        conn: &impl ConnectionTrait,
        ctx: &WorkspaceCtx,
        comment_id: CommentId,
        targets: Vec<CommentLinkTarget>,
    ...
    for candidate in candidates {
        match candidate {
            CommentLinkCandidate::Task { readable_id } => {
                if let Some(task) = crate::entities::boards_tasks::task::Entity::find()
                    .filter(
                        crate::entities::boards_tasks::task::Column::WorkspaceId
                            .eq(ctx.workspace_id.0),
                    )
                    .filter(
                        crate::entities::boards_tasks::task::Column::ReadableId
                            .eq(readable_id.as_str()),
                    )
                    .filter(crate::entities::boards_tasks::task::Column::DeletedAt.is_null())
                    .one(conn)
                    .await
                    .map_err(db_err)?
                {
            ...
            CommentLinkCandidate::Note { slug } => {
                if let Some(document) = crate::entities::documents::document::Entity::find()
                    .filter(
                        crate::entities::documents::document::Column::WorkspaceId
                            .eq(ctx.workspace_id.0),
                    )
                    .filter(crate::entities::documents::document::Column::Slug.eq(slug.as_str()))
                    .filter(crate::entities::documents::document::Column::DeletedAt.is_null())
                    .one(conn)
                    .await
                    .map_err(db_err)?
                {
            ...
            CommentLinkCandidate::Uuid(id) => {
                if crate::entities::documents::document::Entity::find_by_id(id)
                    .filter(
                        crate::entities::documents::document::Column::WorkspaceId
                            .eq(ctx.workspace_id.0),
                    )
                ...
                } else if crate::entities::boards_tasks::task::Entity::find_by_id(id)
                    .filter(
                        crate::entities::boards_tasks::task::Column::WorkspaceId
                            .eq(ctx.workspace_id.0),
                    )
    ...
    if entry_type == "comment" {
        return Ok(CommentFeedEntry::Comment(
            atlas_acta::entities::comments::Comment {
                ...
                created_by: crate::entities::boards_tasks::actor_from_columns(
