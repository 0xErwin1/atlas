...
impl MigrationName for Migration {
    fn name(&self) -> &str {
        "m20260620_000014_saved_searches"
    }
}
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        conn.execute_unprepared(
            r#"CREATE UNIQUE INDEX saved_searches_ws_owner_user_name_idx
               ON saved_searches (workspace_id, owner_user_id, lower(name))
               WHERE deleted_at IS NULL AND owner_user_id IS NOT NULL"#,
        )
        .await?;

        conn.execute_unprepared(
            r#"CREATE UNIQUE INDEX saved_searches_ws_owner_key_name_idx
               ON saved_searches (workspace_id, owner_api_key_id, lower(name))
               WHERE deleted_at IS NULL AND owner_api_key_id IS NOT NULL"#,
        )
        .await?;

        conn.execute_unprepared(
            r#"CREATE INDEX saved_searches_owner_user_idx
               ON saved_searches (workspace_id, owner_user_id) WHERE deleted_at IS NULL"#,
        )
        .await?;

        conn.execute_unprepared(
            r#"CREATE INDEX saved_searches_owner_key_idx
               ON saved_searches (workspace_id, owner_api_key_id) WHERE deleted_at IS NULL"#,
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        conn.execute_unprepared("DROP TABLE IF EXISTS saved_searches CASCADE")
            .await?;
