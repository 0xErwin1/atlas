...
pub(crate) struct Operation {
    ...
    /// The shell component this operation belongs to (design D5).
    pub(crate) component: Component,
    pub(crate) summary: &'static str,
    pub(crate) schema: fn() -> Value,
}

/// One of the three shell components (design D5): `acta`, `custos`, or
/// `platform`. Declared per operation alongside `legacy_name`; audited
/// against the sub-client namespace the operation's own handler calls
/// through by the derivation walk in
...
/// The operations declared to a component that calls no sub-client, mirroring
/// `atlas_cli::component::NO_CALL_SITE` (design D3.2): closed and justified.
/// A listed entry that starts calling a sub-client, or an unlisted
/// zero-call-site operation, both fail the derivation audit.
///
/// No production code reads this table yet — PR5's MCP derivation audit is
/// its consumer — so a non-test `cargo check` sees it as dead code.
#[allow(dead_code)]
pub(crate) const NO_CALL_SITE: &[(&str, &str, &str)] = &[(
    "identity",
    "ping",
    "answers \"pong\" locally, no sub-client call; platform owns identity/ping (D3.2)",
)];
...
mod tests {
    ...
    fn every_operation_has_a_component_and_the_measured_split_is_pinned() {
        ...
        let acta = OPERATIONS
            .iter()
            .filter(|op| op.component == Component::Acta)
            .count();
        ...
        let platform = OPERATIONS
            .iter()
            .filter(|op| op.component == Component::Platform)
            .count();
    ...
    fn the_three_custos_operations_and_the_one_platform_operation_are_named() {
        ...
        let platform_names: Vec<(&str, &str)> = OPERATIONS
            .iter()
            .filter(|op| op.component == Component::Platform)
            .map(|op| (op.verb, op.resource))
            .collect();
        assert_eq!(platform_names, vec![("identity", "ping")]);
    }

    #[test]
    fn no_call_site_is_closed_to_identity_ping_and_matches_its_declared_component() {
        assert_eq!(NO_CALL_SITE.len(), 1);
        let (verb, resource, reason) = *NO_CALL_SITE.first().unwrap_or_else(|| unreachable!());
        assert_eq!((verb, resource), ("identity", "ping"));
        assert!(!reason.is_empty());

        let op = find_operation(verb, resource).unwrap_or_else(|| unreachable!());
        assert_eq!(op.component, Component::Platform);
    }

    #[test]
    fn component_parse_round_trips_through_as_str() {
        for component in [Component::Acta, Component::Custos, Component::Platform] {
    ...
    #[test]
    fn an_unknown_component_error_lists_the_real_ones() {
        let message = unknown_component("nope");

        assert!(message.contains("unknown component `nope`"), "{message}");
        assert!(message.contains("acta"), "{message}");
        assert!(message.contains("custos"), "{message}");
