...
use atlas_core::attribution::{ActorTypeFilter, ApiKeyAttributionId, UserAttributionId};
...
use crate::entities::security_audit::security_audit_log;
...
fn actor_columns(actor: &Attribution) -> (Option<uuid::Uuid>, Option<uuid::Uuid>) {
    match actor {
        Attribution::User(uid) => (Some(uid.0), None),
        Attribution::ApiKey(kid) => (None, Some(kid.0)),
    }
}

fn actor_from_row(user_id: Option<uuid::Uuid>, api_key_id: Option<uuid::Uuid>) -> Attribution {
...
mod tests {
    ...
    fn user_actor_round_trips_through_xor_columns() {
        let actor = Attribution::User(UserAttributionId(UserId::new().0));
    ...
    fn api_key_actor_round_trips_through_xor_columns() {
        let actor = Attribution::ApiKey(ApiKeyAttributionId(ApiKeyId::new().0));
        let (user_col, key_col) = actor_columns(&actor);
