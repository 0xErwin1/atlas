...
use atlas_acta::ports::documents::FolderPresence;
use atlas_api::{
    dtos::boards_tasks::{
...
#[utoipa::path(
    post,
    path = "/workspaces/{ws}/projects/{project_slug}/documents",
    tag = "documents",
...
) -> Result<atlas_acta::entities::documents::Document, ApiError> {
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/compact",
    tag = "documents",
    security(("bearer_auth" = [])),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/content/range",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
...
#[utoipa::path(
    post,
    path = "/workspaces/{ws}/documents/{slug}/content/search",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(("ws" = String, Path), ("slug" = String, Path)),
...
fn resolve_search_continuation(
    ...
    document: &atlas_acta::entities::documents::Document,
...
fn bounded_search(
    ...
    for (_, text) in atlas_acta::document_lines::document_lines(&content[line_start..]) {
...
fn bounded_range(
    ...
    for (line_number, text) in atlas_acta::document_lines::document_lines(content) {
...
fn range_has_more(content: &str, next_line: u64, next_byte: u64, end_line: u64) -> bool {
    atlas_acta::document_lines::document_lines(content).any(|(line_number, text)| {
        let line_number = line_number as u64;
...
#[utoipa::path(
    put,
    path = "/workspaces/{ws}/documents/{slug}/content",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
pub(crate) async fn update_content(
    ...
    let updated = doc_svc
        ...
        .await
        .map_err(|e| match e {
            atlas_core::error::DomainError::Conflict(c) => ApiError::RevisionConflict(c),
    ...
    let updated = state
        ...
        .map_err(|error| match error {
            atlas_core::error::DomainError::Conflict(conflict) => {
                ApiError::RevisionConflict(conflict)
            }
            other => ApiError::Domain(other),
        })?;
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/history",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/revisions/{seq}",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/backlinks",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/frontmatter",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    post,
    path = "/workspaces/{ws}/documents/{slug}/attachments",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/comments/{comment_id}/attachments/{attachment_id}",
...
#[utoipa::path(
    patch,
    path = "/workspaces/{ws}/documents/{slug}/move",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
fn document_move_batch_problem(index: usize, error: ApiError) -> DocumentMoveBatchResultDto {
    let (status, r#type, title, hint) = match error {
        ApiError::InvalidInput { .. }
        | ApiError::Domain(atlas_core::error::DomainError::InvalidInput { .. }) => (
            422,
        ...
        ApiError::Conflict
        | ApiError::Domain(atlas_core::error::DomainError::AlreadyExists { .. }) => (
            409,
...
#[utoipa::path(
    post,
    path = "/workspaces/{ws}/documents/{slug}/copy",
    tag = "documents",
    security(("bearer_auth" = [])),
    params(
        ("ws" = String, Path, description = "Workspace slug"),
...
pub(crate) async fn copy_document_into(
    ...
    ctx: &WorkspaceCtx,
    doc_svc: &DocumentService,
    source: &atlas_acta::entities::documents::Document,
    folder_id: Option<FolderId>,
    project_id: Option<atlas_acta::ids::ProjectId>,
) -> Result<atlas_acta::entities::documents::Document, ApiError> {
...
#[utoipa::path(
    post,
    path = "/workspaces/{ws}/documents/{slug}/comment-drafts",
...
#[utoipa::path(
    delete,
    path = "/workspaces/{ws}/documents/{slug}/comment-drafts/{draft_id}",
...
#[utoipa::path(
    get,
    path = "/workspaces/{ws}/documents/{slug}/comments",
...
#[utoipa::path(
    patch,
    path = "/workspaces/{ws}/documents/{slug}/comments/{comment_id}",
...
fn document_to_dto(doc: atlas_acta::entities::documents::Document) -> DocumentDto {
    DocumentDto {
        id: doc.id.0,
...
fn document_to_compact_dto(doc: atlas_acta::entities::documents::Document) -> DocumentCompactDto {
...
fn document_preview(head: &str) -> Option<String> {
    let (_, body) = atlas_acta::frontmatter::strip_frontmatter(head);
...
fn attachment_to_dto(a: atlas_acta::entities::documents::Attachment) -> AttachmentDto {
    AttachmentDto {
        id: a.id.0,
