//! Background tasks that keep the board presence registry fresh.
//!
...
//!   older than [`PRESENCE_TTL`] and re-broadcasts the affected boards;
//! - [`run_presence_agent_consumer`] watches the live-event stream and marks an
//!   api-key principal present on a board for as long as it keeps mutating it,
...
use super::broadcast_board_presence;
...
/// broadcasts a fresh snapshot for every board whose visible set changed.
///
/// Runs until `shutdown` flips to `true`. The acting project is unknown at sweep
/// time, so `broadcast_board_presence` is called with `None`; the SSE filter then
/// resolves the board's project itself, which is acceptable at this cadence.
...
/// Marks an api-key principal present on a board for as long as it keeps mutating
/// it, driven entirely by the live-event stream.
///
/// Subscribes once to the live hub and, until `shutdown` flips to `true`, refreshes
/// the acting agent's presence on every qualifying event's board. A genuinely new
...
/// establishing the acting agent's presence on the event's board when applicable.
///
/// Events are ignored unless they carry a board id and an api-key actor.
/// `presence.*` events are always skipped so the consumer never reacts to its own
/// broadcasts, which would otherwise form a feedback loop.
async fn handle_agent_event(
    ...
    let Some(board) = event.board_id else {
        return;
    };
    ...
    let Some(actor_dto) = resolve_agent_actor(state, resolved, event.workspace_id, actor_ref.id).await
    else {
        return;
    };

    let changed = state.presence.heartbeat(event.workspace_id, board, actor_dto);
    if changed {
        broadcast_board_presence(state, event.workspace_id, board, event.project_id);
    }
}
