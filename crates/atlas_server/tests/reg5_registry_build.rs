...
fn declared_route_count_matches_the_live_router_enumeration() {
    ...
    // raising the total to 221. v2-e4-s3a-agents added custos's five
    ...
    // total to 242. v2-e5-s6b-authorize-route added `POST /authorize` and
    // `POST /authorize/batch`, raising the total to 244.
    //
    ...
    assert_eq!(
        ...
        "platform + custos + acta declared routes must equal the live router's 244 representable (method, path) pairs"
    );
}
