...
//! call count to 220 across `lib.rs`/`custos.rs`/`acta.rs`/`platform.rs`.
...
//! are pinned separately: the 220 verb-bearing routes are the cross-check
//! against the route contract, and [`WRAPPER_ROUTE_COUNT`] is the number of
//! wrappers on top of them, so `client_routes()` carries `220 + 3` method
...
pub(crate) const CROSS_CHECK_EXTRACTED_CALL_COUNT: usize = 220;
