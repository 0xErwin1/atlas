...
/// measured baseline): 161 path keys, 242 operations, every one of them
...
/// (153 = 145 + the four agent-principal lifecycle path keys — the five
/// routes share one key, `/custos/agents` — plus the two v2-e4-s3b key-family
/// path keys `/custos/personal-api-keys(/{key_id}(/grants(/{grant_id})))` and
...
/// 230 + their ten operations; 161 = 159 + the two
/// v2-e5-s6b-authorize-route path keys `/custos/authorize(/batch)`; 242 =
/// 240 + their two operations.)
#[test]
