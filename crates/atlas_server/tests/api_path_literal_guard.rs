...
// `Literal`, `Scanned`, and `scan` (the comment/string-masking tokenizer)
// live in `support::scan` (`v2-e11-s4` PR2), shared with
// `client_call_shape_guard.rs` so both source-walking gates use the same
// masking discipline instead of two independently-maintained tokenizers.
use support::scan::{Scanned, scan};
