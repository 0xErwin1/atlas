...
//! unlisted file with a matching literal FAILS. A marker pair without a
//! `MarkedSpans` entry exempts nothing, so the allowlist stays the single
//! place a reason lives.
...
/// first matching literal, a `MarkedSpans` file's first matching literal
...
fn violations(root: &Path, allowed: &[Exemption]) -> Vec<String> {
    ...
    for (file, literals) in &files {
        ...
        match (exemption, literals.unmarked.first()) {
            (None, Some(line)) => violations.push(format!(
                "{file}:{line}: matching literal in a file with no allowlist entry"
            )),
            (Some(Exemption::MarkedSpans { .. }), Some(line)) => violations.push(format!(
                "{file}:{line}: matching literal outside a marked span (MarkedSpans entry)"
            )),
            _ => {}
        }
    }
