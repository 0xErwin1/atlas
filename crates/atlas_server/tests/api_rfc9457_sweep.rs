...
async fn every_declared_route_answers_with_a_conformant_problem_body() {
    ...
    for entry in &matrix {
        ...
        let (expected_status, request_id, response) = match provocation {
            ...
            Provocation::Unauthenticated401 => {
                let request_id = format!("sweep-{}-{}", entry.method, path);
                let response = http
            ...
            Provocation::LoginBadCredentials401 => {
                let request_id = "sweep-login-bad-credentials".to_string();
                let response = http
                    .post(&url)
                    .header("x-request-id", request_id.clone())
            ...
            Provocation::UnknownActivationToken404 => {
                let request_id = format!("sweep-activate-{}", entry.method);
            ...
            Provocation::MissingWebhookSignature401 => {
                let request_id = "sweep-ingest-missing-signature".to_string();
                let response = http
                    .post(&url)
                    .header("x-request-id", request_id.clone())
        ...
        assert_eq!(
            ...
            "expected {expected_status} for {} {} but got {status}",
            entry.method, path
        );
        ...
        assert_eq!(
            ...
            "expected application/problem+json for {} {} but got {content_type:?}",
            entry.method,
            path
        );
        ...
        let body: serde_json::Value = response
            .json()
            .await
            .unwrap_or_else(|e| panic!("body for {} {} must be JSON: {e}", entry.method, path));

        for field in ["type", "title", "status", "request_id", "instance"] {
            assert!(
                body.get(field).is_some(),
                "problem body for {} {} is missing required field `{field}`: {body:?}",
                entry.method,
                path
            );
        }
        ...
        assert_eq!(
            ...
            "request_id in the problem body for {} {} does not match the x-request-id header sent",
            entry.method,
            path
        );
    }
    ...
    assert!(
        ...
        "the declared route set and the provoked-or-excluded route set disagree; \
         declared but not provoked or excluded: {declared_but_not_provoked_or_excluded:?}; \
         provoked or excluded but not declared: {provoked_or_excluded_but_not_declared:?}"
    );
