...
/// migration; `workspaces` (PR11, already moved) stays `acta`, and every
/// other batch's table (still unbatched in this PR) stays in `public`,
/// including `boards` (batch 3) and `purge_operations` (batch 5).
#[tokio::test]
async fn documents_group_tables_move_to_the_acta_schema() {
    ...
    for table in ACTA_DOCUMENTS_TABLES.iter().chain(["workspaces"].iter()) {
        let schema = rows
            .iter()
...
/// `boards_project_id_fkey` (an Acta table not yet batched, still `public`,
/// referencing the now-moved `acta.projects`), and
