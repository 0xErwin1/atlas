...
//! final S3 split, now 40/195) and the composed document's annotated-operation
...
/// re-key do not land in the same commit, every one of the 40 annotations
/// disappears silently.
...
/// The 40 routes T6.8 asserts carry the full annotation set.
...
/// T6.9: confirmed against the merged split (35 routes before
/// `v2-e4-s3b-key-families` added `POST /personal-api-keys` and
/// `POST /agent-api-keys`, 40 after).
#[test]
fn exactly_37_routes_are_declared_idempotent() {
    let declared = declared_idempotent_routes();
    assert_eq!(
        declared.len(),
        40,
        "expected exactly 40 idempotent: true routes, found {}: {:?}",
        declared.len(),
        declared
    );
}

/// Placement audit over all 40 idempotent operations: every response entry,
...
fn the_bidirectional_audit_detects_a_drifted_route() {
    ...
    let dropped = annotated
        ...
        .next()
        .cloned()
        .expect("the 40-route idempotent set must be non-empty");
...
/// every one of the 40 operations' `409` idempotency-conflict response
